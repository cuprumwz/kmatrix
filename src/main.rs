...
impl Keyboard for KMatrix {
    const MANUFACTURER: &'static str = ""; // TODO: Change this
    const PRODUCT: &'static str = "KMatrix";
...
impl KeyboardLayout for KMatrix {
...
impl KeyboardMatrix for KMatrix {
    build_matrix! {
...
impl ViaKeyboard for KMatrix {
    // Uncomment if using `storage`
    // rumcake::setup_via_storage_buffers!(KMatrix);
}
use rumcake::vial::VialKeyboard;
impl VialKeyboard for KMatrix {
    ...
    // Uncomment if using `storage`
    // rumcake::setup_vial_storage_buffers!(KMatrix);
}
